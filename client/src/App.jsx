...
function App() {
  ...
  const loadContracts = (signer) => {
    ...
    setUsers(users1);
    const market1 = new ethers.Contract(
      MarketAddress.address,
      Market.abi,
      signer
    );
    setMarket(market1);
    const fProduct1 = new ethers.Contract(
      FProductAddress.address,
      FProduct.abi,
      signer
    );
    setFProduct(fProduct1);
    const dProduct1 = new ethers.Contract(
      DProductAddress.address,
      DProduct.abi,
      signer
    );
      ...
      <div>
        {loading ? (
          <Routes>
            <Route
              path="/"
              element={<Home web3Handler={web3Handler} account={account} />}
            />
          </Routes>
        ) : (
          <div>
            <NavigationBar account={account} loading={loading} users={users} />
            {isUser || isDesigner || isManufacturer ? (
              <NavigationBar account={account} loading={loading} users={users}/>
            ) : (
              <Routes>
                <Route
                ...
                <Route
                  path="/registerPage"
                  element={
                    <Register users={users} account={account} setIsUser={setIsUser} setIsDesigner={setIsDesigner} setIsManufacturer={setIsManufacturer} />
                  }
                />
              </Routes>
            )}
